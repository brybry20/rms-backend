...
def init_db():
    ...
    try:
        if Config.USE_POSTGRES:
            ...
            # Add missing columns for existing databases (with error handling)
            try:
                cursor.execute("ALTER TABLE dealer_profiles ADD COLUMN IF NOT EXISTS region TEXT")
                print("✅ region column ready")
            except Exception as e:
                print(f"Note: region column: {e}")
            
            try:
                cursor.execute("ALTER TABLE dealer_profiles ADD COLUMN IF NOT EXISTS province TEXT")
                print("✅ province column ready")
            except Exception as e:
                print(f"Note: province column: {e}")
